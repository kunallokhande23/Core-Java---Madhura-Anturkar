		...
		try {
			for (int i = 0; i < 10; i++) {
				...
				if(currentThread().getName().equals("three"))
				{
					...
					System.in.read();//tries to read 1 byte from keyboard(stdin) : t3 : Blocked on i/p
				}
					Thread.sleep(500);//t3 : Blocked on sleep --gets pending interrupt signal --sleep interrupted --- un blcoked --rdy --runing -- IntExc 
			}
		} catch (Exception e) {
